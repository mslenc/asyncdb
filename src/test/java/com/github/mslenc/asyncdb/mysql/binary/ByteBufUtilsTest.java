...
public class ByteBufUtilsTest {
    ...
    public void testToHexString() {
        assertTrue("cafebabe".equalsIgnoreCase(
            ByteBufUtils.toHexString(new byte[] {(byte)  0xca, (byte) 0xfe, (byte) 0xba, (byte) 0xbe})));

        assertTrue("".equalsIgnoreCase(ByteBufUtils.toHexString(new byte[0])));

        assertTrue("0000000000".equalsIgnoreCase(ByteBufUtils.toHexString(new byte[] { 0, 0, 0, 0 ,0 })));

        try {
            ByteBufUtils.toHexString(null);
    ...
    public void testIsNullBitSet() {
        ...
        for (int i = 0; i < expect.length(); i++) {
            ...
            for (int offset = 0; offset <= i; offset++) {
                Assert.assertEquals(expectBit, ByteBufUtils.isNullBitSet(offset, nullBytes, i - offset));
            }
        }
    }
    ...
    public void testSetNullBit() {
        ...
        ByteBufUtils.setNullBit(0, bytes, 0);
        assertTrue(ByteBufUtils.isNullBitSet(0, bytes, 0));
        assertEquals(1, countOneBits(bytes));

        ByteBufUtils.setNullBit(2, bytes, 3);
        assertTrue(ByteBufUtils.isNullBitSet(0, bytes, 0));
        assertTrue(ByteBufUtils.isNullBitSet(2, bytes, 3));
        assertEquals(2, countOneBits(bytes));

        ByteBufUtils.setNullBit(0, bytes, 7);
        assertTrue(ByteBufUtils.isNullBitSet(0, bytes, 0));
        assertTrue(ByteBufUtils.isNullBitSet(2, bytes, 3));
        assertTrue(ByteBufUtils.isNullBitSet(0, bytes, 7));
        assertEquals(3, countOneBits(bytes));

        ByteBufUtils.setNullBit(2, bytes, 45);
        assertTrue(ByteBufUtils.isNullBitSet(0, bytes, 0));
        assertTrue(ByteBufUtils.isNullBitSet(2, bytes, 3));
        assertTrue(ByteBufUtils.isNullBitSet(0, bytes, 7));
        assertTrue(ByteBufUtils.isNullBitSet(2, bytes, 45));
    ...
    public void testReadFixedBytes() {
        ...
        for (int len = 0; len < 500; len += 13) {
            ...
            for (int read = 0; read <= len + 10; read += rnd.nextInt(17)) {
                ...
                byte[] readBack = ByteBufUtils.readFixedBytes(buf, read);
    ...
    public void testWriteCString() {
        ...
        ByteBufUtils.writeCString(buf1, "", UTF_8);
        ByteBufUtils.writeCString(buf2, "123", UTF_8);
        ByteBufUtils.writeCString(buf3, "lorem ipsum sit ", UTF_8);
        ByteBufUtils.writeCString(buf4, "0\u007f\u00FF", UTF_8);
        ByteBufUtils.writeCString(buf5, "Mitja Šlenc", UTF_8);
        ByteBufUtils.writeCString(buf6, "\uD83D\uDEC8", UTF_8);
    ...
    public void testReadUntilEOFOrZero() {
        ...
        for (String expected : tests) {
            ...
            String readBack = ByteBufUtils.readUntilEOFOrZero(buf, UTF_8);
            ...
            buf2.writeByte(0);
            String readBack2 = ByteBufUtils.readUntilEOFOrZero(buf2, UTF_8);
            ...
            String readBack3 = ByteBufUtils.readUntilEOFOrZero(buf3, UTF_8);
    ...
    public void testReadUntilEOF() {
        ...
        for (String expected : tests) {
            ByteBuf buf = Unpooled.buffer();
            buf.writeCharSequence(expected, UTF_8);
            String readBack = ByteBufUtils.readUntilEOF(buf, UTF_8);
            assertEquals(expected, readBack);
            ...
            buf2.writeCharSequence(expected, UTF_8);
            buf2.writeByte(0);
            String readBack2 = ByteBufUtils.readUntilEOF(buf2, UTF_8);
            assertEquals(expected + "\u0000", readBack2);
            ...
            buf3.writeByte(0);
            buf3.writeBytes(new byte[] { 'a', 'b', 'c' });
            String readBack3 = ByteBufUtils.readUntilEOF(buf3, UTF_8);
    ...
    public void testReadCString() {
        ...
        for (String expected : tests) {
            ByteBuf buf = Unpooled.buffer();
            buf.writeCharSequence(expected, UTF_8);
            String readBack = ByteBufUtils.readCString(buf, UTF_8);
            assertNull(readBack);
            ...
            buf2.writeCharSequence(expected, UTF_8);
            buf2.writeByte(0);
            String readBack2 = ByteBufUtils.readCString(buf2, UTF_8);
            assertEquals(expected, readBack2);
            assertEquals(0, buf2.readableBytes());
            ...
            buf3.writeByte(0);
            buf3.writeBytes(new byte[] { 'a', 'b', 'c' });
            String readBack3 = ByteBufUtils.readCString(buf3, UTF_8);
            assertEquals(expected, readBack3);
    ...
    public void testWriteLength() {
        ...
        for (Map.Entry<Long, byte[]> entry : tests.entrySet()) {
            ...
            ByteBuf buf = Unpooled.buffer();
            ByteBufUtils.writeLength(value, buf);

            byte[] actual = new byte[buf.readableBytes()];
            buf.readBytes(actual);

            assertTrue(Long.toHexString(value) + ": " + ByteBufUtils.toHexString(expected) + " vs " + ByteBufUtils.toHexString(actual),
    ...
    public void testWriteLengthEncodedString() {
        ...
        for (Map.Entry<String, byte[]> entry : tests.entrySet()) {
            ...
            ByteBuf buf = Unpooled.buffer();
            ByteBufUtils.writeLengthEncodedString(text, UTF_8, buf);

            byte[] actual = new byte[buf.readableBytes()];
            buf.readBytes(actual);

            assertTrue(text + ": " + ByteBufUtils.toHexString(expected) + " vs " + ByteBufUtils.toHexString(actual),
                       Arrays.equals(expected, actual));
        }

        // let's also check choosing charset is not ignored
        ByteBuf buf = Unpooled.buffer();
        ByteBufUtils.writeLengthEncodedString("Iñtërnâtiônàlizætiøn", ISO_8859_1, buf);
        byte[] actual = new byte[buf.readableBytes()];
        buf.readBytes(actual);
        ...
        assertTrue("Iñtërnâtiônàlizætiøn: " + ByteBufUtils.toHexString(expected) + " vs " + ByteBufUtils.toHexString(actual),
                Arrays.equals(expected, actual));
    }
    ...
    public void testReadBinaryLength() {
        ...
        for (long value : tests) {
            // test plain read
            ByteBuf buf1 = Unpooled.buffer();
            ByteBufUtils.writeLength(value, buf1);
            long actual = ByteBufUtils.readBinaryLength(buf1);
            assertEquals(value, actual);
            assertEquals(0, buf1.readableBytes());

            // test read with first byte pre-read
            ByteBuf buf2 = Unpooled.buffer();
            ByteBufUtils.writeLength(value, buf2);
            int firstByte = buf2.readUnsignedByte();

            long actual2 = ByteBufUtils.readBinaryLength(firstByte, buf2);
            ...
            ByteBufUtils.writeLength(value, buf3);

            for (int a = 0; a < suffix; a++)
                buf3.writeByte(rnd.nextInt());

            buf3.skipBytes(prefix);
            long actual3 = ByteBufUtils.readBinaryLength(buf3);
    ...
    public void testReadBinaryLengthSpecialCases() {
        ...
        Assert.assertEquals(-1, ByteBufUtils.readBinaryLength(Unpooled.wrappedBuffer(bytes(251))));
        Assert.assertEquals(-2, ByteBufUtils.readBinaryLength(Unpooled.wrappedBuffer(bytes(255))));
        ...
        for (int first = 0; first <= 255; first++) {
            ...
            long readBack = ByteBufUtils.readBinaryLength(buf);
    ...
    public void testReadLengthEncodedString() {
        ...
        for (int targetLen : byteLengths) {
            ...
            ByteBufUtils.writeLengthEncodedString(expected, UTF_8, buf);
            String actual = ByteBufUtils.readLengthEncodedString(buf, UTF_8);
        ...
        for (int targetLen : byteLengths) {
            ...
            ByteBuf buf = Unpooled.buffer();
            String expected = sb.toString();
            ByteBufUtils.writeLengthEncodedString(expected, ISO_8859_1, buf);
